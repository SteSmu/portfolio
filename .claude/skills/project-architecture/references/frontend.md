...
| [`components/charts/EquityCurve.tsx`](../../frontend/src/components/charts/EquityCurve.tsx) | ECharts line — total value + cost-basis dashed overlay + optional `benchmark` prop (start-aligned ratio-scaled overlay in `var(--cat-3)`). Pulls value+cost via [`lib/snapshotSeries.ts:pickEquitySeries`](../../frontend/src/lib/snapshotSeries.ts) which prefers `total_value_base` and FX-converts cost basis when every visible snapshot has a base value. |
| [`components/charts/DrawdownChart.tsx`](../../frontend/src/components/charts/DrawdownChart.tsx) | ECharts area chart, peak-to-trough %, always negative. Same `pickEquitySeries` helper as EquityCurve so DD reflects base-currency moves (FX + price) when available. |
| [`lib/snapshotSeries.ts`](../../frontend/src/lib/snapshotSeries.ts) | `pickEquitySeries(snapshots) → {mode, currency, values, costs}` + `drawdownFromValues(values)`. Single source of truth for "should this chart show base currency or FX-naive sums". |
...
| [`components/charts/AssetPriceChart.tsx`](../../frontend/src/components/charts/AssetPriceChart.tsx) | lightweight-charts v5 line + `createSeriesMarkers` (buy/sell/transfer arrows + optional sentiment-tinted news dots, single plugin instance reused via `setMarkers`) + `createPriceLine` (cost-basis dashed, removed before re-adding to avoid stacking). `chart.subscribeClick` snaps to the nearest news pin within ±1 day and opens the article URL. |
