...
| [`Allocation.tsx`](../../frontend/src/pages/Allocation.tsx) | `/allocation` | ECharts **sunburst** (asset_type → currency → symbol) with focus-on-ancestor drilldown; donut variant; per-asset-class breakdown cards | `listHoldings` |
...
| [`components/BenchmarkPicker.tsx`](../../frontend/src/components/BenchmarkPicker.tsx) | `<select>` over `GET /api/benchmarks` (SPY / URTH / IWDA / QQQ + "none"). Selection persisted in localStorage under `pt:benchmark` via [`state/benchmark.ts`](../../frontend/src/state/benchmark.ts). Mounted on Dashboard + Performance equity-curve cards. |
